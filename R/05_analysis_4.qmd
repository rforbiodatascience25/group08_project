...
```{r}
ggplot(newdata, aes(x = time, y = fit, color = condition)) +
  geom_line(size = 1) +
  geom_ribbon(aes(ymin = lower, ymax = upper, fill = condition), alpha = 0.15, color = NA) +
  facet_wrap(~ tissue) +
  labs(title = paste0("ATGPT2: model predicted expression over time (Shaded = 95% CI)"),
...
print(paste0("Annotated genes have ", length(cold_and_stress_gene_names), " entries"))
...
print(paste0("Model genes have ", length(model_gene_names), " entries"))
