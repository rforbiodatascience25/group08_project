...
readr::write_tsv(expr_annotated, "../_raw/data/expression_annotated.csv")
```
...
readr::write_tsv(expr_filtered, "../_raw/data/expression_filtered_normalized.csv")
...
# Gene symbols often appear multiple times
safe_symbols <- make.unique(expr$symbol)
rownames(expr_only) <- safe_symbols

# 3. Transpose matrix: samples as rows
expr_t <- t(expr_only)
expr_t <- as.data.frame(expr_t)

# Add GSM ID column for merging
expr_t$GSM <- rownames(expr_t)

# 4. Merge metadata (NOW expr_with_meta exists!)
...
readr::write_tsv(expr_with_meta, "../_raw/data/expression_with_metadata.csv")
